...
public:
  ...
  FactorTable reduce(const map<RandomVariable, int> var_vals) const{ // reduced table, given vals for some vars. var_vals need not be a subset of vars.
    FactorTable res;
    auto final_vars = vars;
    vector<int> vals(vars.size(),0);

    for(const auto &var_val: var_vals){ //remove vars to be reduced from final
      if(final_vars.erase(var_val.first)) //if var doesn't exist in final_vars, ignore it.
        vals[var_mapping.at(var_val.first.id)] = var_val.second; //populate val for reducted vars
    }

    res.init(final_vars);

    size_t idx = get_idx(vals); // non reduces will iterate, reduced fixed.
  ...
  pair<double, int> maximal_val(RandomVariable var) const {
    auto to_sum = RandomVariable::set_difference(vars, {var});
    auto marginal = sum(to_sum);
    double max=0, sum=0; int arg_max=-1;
    assert(marginal.table.size() == var.card);
  ...
  double logp(RandomVariable var, int val) const {
    assert(val < var.card);
    auto to_sum = RandomVariable::set_difference(vars, {var});
    auto marginal = sum(to_sum);
    double sum=0;
