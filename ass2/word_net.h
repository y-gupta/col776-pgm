...
public:
  ...
  WordNet(){
    ...
    loopy = 0;//0: clique tree + MP, 1: bethe + loopy
  ...
  void read_phi(const char *ocr,const char *trans){
    ...
    for(int i=0;i<1000;i++){
      phi_ocr_reduced.push_back(phi_ocr.reduce({{RandomVariable(0),i}}));
    }
  }
  ...
  void process_word_models(const char *fname_img, const char *fname_word){
    ...
    while(1){
      ...
      if(!loopy){
        cluster_graph = make_clique_tree(net);
      }else{
        cluster_graph = make_bethe_graph(net);
      }
      auto words = word1;
      words.insert(words.end(),word2.begin(), word2.end()); //concat word1,word2
      timer.reset();
      auto words_logp = infer(cluster_graph, words); //inferred_word1 and word2 (concat'd), #logp for given words
  ...
  ClusterGraph make_clique_tree(MarkovNet &net){
    ClusterGraph ctree;
    auto ordering = net.get_ordering();
    ctree.to_clique_tree(net, ordering);
    return ctree;
  }
  ClusterGraph make_bethe_graph(MarkovNet &net){
    ClusterGraph bgraph;
    bgraph.to_bethe_graph(net);
    return bgraph;
  }
  pair<vector<char>, double> infer(ClusterGraph &cgraph, vector<char> &words){
    pair<vector<char>, double> res;
    map<RandomVariable, int> predicted;
    map<RandomVariable, int> correct;
    for(int i=0;i<words.size();i++){
      correct[RandomVariable(i,10)] = char_map[words[i]];
    }
    if(loopy){
      BetheGraphLBPInferer inferer;
      inferer.run(cgraph);
      res.second = inferer.infer(predicted);
      res.second = inferer.logLikelihood(correct);
    }else{
      CliqueTreeMPInferer inferer;
      inferer.run(cgraph);
      res.second = inferer.infer(predicted);
      res.second = inferer.logLikelihood(correct);
    }
    res.first.resize(cgraph.clusters.size());
    for(auto &var_val: predicted)
        res.first[var_val.first.id] = chars[var_val.second];
    return res;
  }
  ...
  ClusterGraph test_cgraph(){
    auto mnet = test_mnet();
    ClusterGraph cgraph;
    if(loopy)
      cgraph = make_bethe_graph(mnet);
    else
      cgraph = make_clique_tree(mnet);

    cgraph.print();
    return cgraph;
  }
  void test_inference(){
    auto cgraph = test_cgraph();
    printf("---inference---\n");
    if(loopy){
      ...
      printf("checked ll: %f\n",ll2);
      for(auto &var_val: predicted){
        printf("  %d: %d\n",var_val.first.id, var_val.second);
      }
      printf("\n");
    }
  }
